...
function getCookie(name) {
   ...
   let valueArray = valueCookie.split(';');
   if (!valueArray[0]) { return undefined };
   let valueObject = {};
...
function TranslateInit(config) {
   ...
   if (config.langFirstVisit && (getCookie('googtrans') === undefined || getCookie('googtrans') === 'null')) {

      /* Если установлен язык перевода для первого посещения и куки не назначены */
      /* If the translation language is installed for the first visit and cookies are not assigned */
      setCookie("/auto/" + config.langFirstVisit);
   }
   ...
   TranslateEventHandler("click", "[data-google-lang]", function (e) {
      setCookie("/" + config.lang + "/" + e.getAttribute("data-google-lang"));
...
function TranslateGetCode(config) {
   ...
   let lang =
      getCookie('googtrans') != undefined && getCookie('googtrans') != "null"
         ? getCookie('googtrans')
         : config.lang;
